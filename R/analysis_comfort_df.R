...
varname_dict = tmp.rename.func(varname_dict, "age_impute", "Age")
varname_dict = tmp.rename.func(varname_dict, "primary_role", "Primary role")
...
# 2. Model coefficients and random effects ----
    ...
    mcmc_intervals(me_per_vid.array, 
                   ...
                   pattern = "person_ID|lp__|r_person|r_video")][c(1:6,7,25,29,8:24,26:28,30:31)], prob = .5, prob_outer = .95) + 
  ...
  source("mcmc_areas_ridges2.R")
    ...
    mcmc_areas_ridges2(me_per_vid.array, 
                      pars = (dimnames(me_per_vid.array)$parameters[!grepl(dimnames(me_per_vid.array)$parameters,
                      pattern = "person_ID\\[|lp__|r_person|r_video")])[c(1:6,7,25,29,8:24,26:28,30:32)], prob = .99, prob_outer = .99,
                     size = .3, names = varname_dict) + 
      geom_vline(aes(xintercept = 0))  + 
      coord_cartesian(xlim=c(-7,7)) +
      scale_x_continuous(breaks=c(-6,-4,-2,0,2,4,6),name="Parameter estimates (log-cumulative-odds)")+
      theme_bw()
    
    ggsave(filename = "../IMG/me_per_vid_ridges.png", width = 6.5, height = 9)
    ...
    which(pi.means < -5 )
    filter(d, person_ID %in% c("21", "2149", "1587", "2060", "44")) %>% dplyr::select(comfort_rating, NCHRP_BLOS_ST, bike_lane_SUM_ST, comfort_four_no_lane, video_name, VideoGroup, person_ID) %>%
      arrange(person_ID)
    # weird that they all have the same videos?
    # I think it suggests that we might get a better estimate of the person level effects
    # if we had a cluster variable that was the video block, not just the video.
...
# 3. Scenarios to understand coefficients ----
    ...
    d.scenario = models$me_per$data %>% group_by(person_ID) %>% dplyr::select(-"comfort_rating_ordered") %>% summarize_all(first)
...
attitudes = data.frame(rbind(bad_attitudes, mid_attitudes, good_attitudes), 
                        id = 1,  attitude = c("bad_attitude", "mid_attitude", "good_attitude"))
...
#example
x = data.frame(c(bad_attitudes, high_ability_comfort, young_childless_male, collector_bad)) #note this doesn't have a person_ID
predict(models$me_per, newdata = x, re_formula = NULL) 
...
lapply(models_fixed, function(x) {x$fixed$Eff.Sample})
